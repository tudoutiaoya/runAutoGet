...
for stu in list:
    ...
    for record in recordList:
        ...
        #测试-------------
        riqi = record.get("beginAt")
        stamp = float(int(riqi) / 1000)
        array = time.localtime(stamp)
        geshiRiqi = time.strftime("%Y-%m-%d", array)
        xuehao = record.get("campusId")
        #测试-------------
        if math.fabs(speed - 10.00) < 0.00000001:
            print(recordList)
            print(record)
            print("学号" + xuehao)
            print("日期：" + geshiRiqi)
            print(record)
        # if(math.fabs(float(speed)-10.00) < 0.000000001):
        #     print(recordList)
        #     print(record)
        #     print(float(speed))
        #     print("学号" + xuehao)
        ...
        # 正常 并且 大于3公里才记录
        if status == 0 and length >= 400 and appealStatus == 0 and (speed>= 4 and speed<10) and (avgStepFreq >= 60 and avgStepFreq <=240):
            # success += 1
    ...
    list[stu] = success
    if i%15 == 0:
        print("爬取进度为: {:.2%}".format(i/len(list)))
    i +=1
